...
describe('CSPARQLWindow', () => {
    ...
    test('add_to_window', () => {
        ...
        const csparqlWindow = new s2r_1.CSPARQLWindow(":window1", 10, 2, s2r_1.ReportStrategy.OnWindowClose, s2r_1.Tick.TimeDriven, 0, 60000);
...
describe('CSPARQLWindow OOO', () => {
    ...
    beforeEach(() => {
        window = new s2r_1.CSPARQLWindow('testWindow', width, slide, s2r_1.ReportStrategy.OnWindowClose, s2r_1.Tick.TimeDriven, startTime, maxDelay);
    });
    afterEach(() => {
    });
    ...
    test('should buffer late elements', () => {
        ...
        window.add(quad2, 0);
        expect(window.late_buffer.size).toBe(1);
        expect((_a = window.late_buffer.get(0)) === null || _a === void 0 ? void 0 : _a.has(quad2)).toBe(true);
    });
    ...
    test('should trigger on window change', (done) => {
        const report_window = new s2r_1.CSPARQLWindow('reportWindow', width, slide, s2r_1.ReportStrategy.OnWindowClose, s2r_1.Tick.TimeDriven, startTime, maxDelay);
...
describe('CSPARQL Window Watermark Test', () => {
    ...
    beforeEach(() => {
        quad1 = {};
        csparqlWindow = new s2r_1.CSPARQLWindow('testWindow', 10, 5, s2r_1.ReportStrategy.OnWindowClose, s2r_1.Tick.TimeDriven, 0, 5);
...
describe('Gamma Operator', () => {
    it('should return the correct gamma value', () => {
        const shape = 4;
        const scale = 3;
        const gamma = (0, s2r_1.gammaOperator)(shape, scale);
        console.log(gamma);
    });
});
